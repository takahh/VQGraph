...
def train_sage(model, dataloader, feats, labels, criterion, optimizer, accumulation_steps=1, lamb=1):
    ...
    for step, (input_nodes, output_nodes, blocks) in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast():  # Mixed precision forward pass
            _, logits, loss, _, cb, loss_list, latent_train = model(blocks, batch_feats)
    ...
    for epoch in range(1, conf["max_epoch"] + 1):
        ...
        if epoch % conf["eval_interval"] == 0:
            if "MLP" in model.model_name:
                ...
                loss_total = float(sum(loss_list1))    #                                              0 raw_feat_loss, 1 raw_edge_rec_loss, 2 raw_commit_loss, 3 spread_loss, 4 margin_loss, 5 pair_loss
            ...
            print(f"train_known_g, epoch {epoch:3d}, feature_loss: {loss_list[0].item(): 4f}| edge_loss: {loss_list[1].item(): 4f}| commit_loss: {loss_list[2].item(): 4f},　margin loss {loss_list[3].item(): 4f}, loss_train {loss:.4f}")
