...
def run_inductive(
    ...
    for epoch in range(1, conf["max_epoch"] + 1):
        ...
        if conf["train_or_infer"] == "train":
            ...
            print(f"train_known_g, div_element_loss: {sum(loss_list_list[0])/len(loss_list_list[0]): 4f}, "
                  f"bond_num_div_loss: {sum(loss_list_list[5])/len(loss_list_list[5]): 4f},"
                  f" aroma_div_loss: {sum(loss_list_list[6])/len(loss_list_list[6]): 4f}, "
                  f"ringy_div_loss: {sum(loss_list_list[7])/len(loss_list_list[7]): 4f}, "
                  f"h_num_div_loss: {sum(loss_list_list[8])/len(loss_list_list[8]): 4f}")
